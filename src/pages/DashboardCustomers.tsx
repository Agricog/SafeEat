...
export default function DashboardMenu() {
  ...
  useEffect(() => {
    let cancelled = false
    request<{ dishes: Array<{ id: string; name: string; description: string; price_pence: number; category: string; allergen_mask: number; ingredients: string; active: boolean }> }>(
  ...
  const handleAdd = async (data: DishFormData) => {
    ...
    setError(null)
    try {
      const res = await request<{ dish: { id: string; name: string; description: string; price_pence: number; category: string; allergen_mask: number; ingredients: string } }>(
        `/api/dashboard/${venueId}/dishes`,
        {
          method: 'POST',
          body: {
  ...
  const handleEdit = async (data: DishFormData) => {
    ...
    setError(null)
    try {
      const res = await request<{ dish: { id: string; name: string; description: string; price_pence: number; category: string; allergen_mask: number; ingredients: string; active: boolean } }>(
