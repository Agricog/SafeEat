 ...
 * Extracts the authenticated user's ID and optionally their venue_id.
 *
 * Sets on context:
 *   c.set('clerkUserId', string)       — Clerk user ID (always set)
 *   c.set('venueId', string | null)    — venue ID from database lookup (null if no venue yet)
 *
 * Note: This middleware no longer blocks users without a venue.
 * Venue enforcement is handled by enforceVenueAccess() on :venueId routes.
 */
export function requireAuth() {
  return async (c, next) => {
    ...
    try {
      ...
      // Look up the venue for this user (optional — new users won't have one)
      ...
      c.set('venueId', venues.length > 0 ? venues[0].id : null)
 ...
 * Also blocks users with no venue from accessing venue-scoped routes.
 */
export function enforceVenueAccess() {
  return async (c, next) => {
    ...
    if (!authVenueId) {
      return c.json({ error: 'No venue linked to this account' }, 403)
    }
